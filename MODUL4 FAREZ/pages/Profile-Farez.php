...
$email=$_GET['email'];
$query = mysqli_query($koneksi, "SELECT * FROM user_farez WHERE email = $email");
...
<body>
    <div class="d-flex justify-content-center align-items-center" style="margin-top: 50px; margin-bottom: 50px;">
        <div class="col-md-3">
            <div class="card">
                <form action="../config/profile.php?email=<?php echo $_SESSION['email']?>" method="POST">
                    <div class="card-body">
                        <h5 class="card-title" style="text-align: left;">Profile</h5>
                        <hr>
                        <div class="mb-3">
                            <label class="form-label" for="email"><b>Email</b></label>
                            <input type="email" class="form-control" name="email" id="email" value="<?php echo $pilih['email']?>" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="user"><b>Nama</b></label>
                            <input type="text" class="form-control" name="user" id="user"  value="<?php echo $pilih['nama']?>" required aria-label="Username">
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="no_hp"><b>Nomor Handphone</b></label>
                            <input type="number" class="form-control" name="no_hp" id="no_hp" value="<?php echo $pilih['no_hp']?>" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="pass"><b>Kata Sandi</b></label>
                            <input type="password" class="form-control" name="pass" id="pass" value="<?php echo $pilih['passwprd']?>" onkeyup="check()" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="repass"><b>Konfirmasi Kata Sandi</b></label>
                            <input type="password" class="form-control" name="repass" id="repass" value="<?php echo $pilih['password']?>" onkeyup="check()" required>
                        </div>
